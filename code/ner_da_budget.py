...
def main(args):
    ...
    if args.budget < pretraining_cost:
        raise ValueError(f"The total budget {args.budget} is less than the pre-training cost {pretraining_cost}.")
    else:
    ...
    train_head_tags, train_head_flags, \
    train_data_types = get_processed_sentences_da_budget([args.src_data, args.tgt_data], 'train',
                                                         annotation_cost, args.max_len, args.batch_size,
                                                         tokenizer, args.alpha)

    dev_dataloader, dev_masks, dev_tags, \
    dev_head_tags, dev_head_flags, \
    dev_data_types = get_processed_sentences_da_budget([None, args.tgt_data], 'dev',
                                                       None, args.max_len, args.batch_size,
                                                       tokenizer, args.alpha)

    test_dataloader, test_masks, test_tags, \
    test_head_tags, test_head_flags, \
    test_data_types = get_processed_sentences_da_budget([None, args.tgt_data], 'test',
                                                       None, args.max_len, args.batch_size,
                                                       tokenizer, args.alpha)
    ...
    for num_epoch in trange(args.epochs, desc="Epoch"):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (args.eval_step > 0 and step % args.eval_step == 0 and step > 0) or \
               (args.eval_step == 0 and step % 100 == 0 and step > 0):
                print("The number of steps: {}".format(step))

            # add batch to gpu
            batch = tuple(t.to(device) for t in batch)
            b_input_ids, b_input_mask, b_labels, b_head_tags, b_head_flags, b_data_types = batch
            # forward pass
            if not USE_HEAD_ONLY:
                loss, _ = model(b_input_ids, token_type_ids=None,
                                    attention_mask=b_input_mask, labels=b_labels, data_types=b_data_types)
            else:
                loss, _ = model(b_input_ids, token_type_ids=None,
                                    attention_mask=b_input_mask, labels=b_labels,
                                    head_tags=b_head_tags, head_flags=b_head_flags, data_types=b_data_types)
        ...
        for batch in dev_dataloader:
            ...
            with torch.no_grad():
                logits = model(b_input_ids, token_type_ids=None,
                                   attention_mask=b_input_mask, data_types=b_data_types)
        ...
        if f1_dev > best_f1:
            ...
            config['best_step'] = len(train_dataloader)
